...
if (!defined("APL_SALT")) {
    ...
    define("APL_LICENSE_FILE_LOCATION", "../license.php");
...
function aplCheckData($MYSQLI_LINK = null)
{
    ...
    if (!empty($ROOT_URL) && !empty($INSTALLATION_HASH) && !empty($INSTALLATION_KEY) && !empty($LCD) && !empty($LRD)) { //do further check only if essential variables are valid
        ...
        if (filter_var(aplGetCurrentUrl(), FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED | FILTER_FLAG_HOST_REQUIRED) && stristr(aplGetRootUrl(aplGetCurrentUrl(), 1, 1, 1, 1), aplGetRootUrl("$ROOT_URL/", 1, 1, 1, 1))===false) { //script is opened via browser (current_url set), but current_url is different from value in database
            $error_detected=1;
        }
